...
const bookList = [
  ...
  const addToList = (x, y) => bookList.push({title: x, author: y});
  addToList("test","book");
  ...
  const listTerms = (x) =>(x === "title") ? bookList.forEach((key) => console.log(title)) : bookList.forEach((key) => console.log(author));
  listTerms("title");
